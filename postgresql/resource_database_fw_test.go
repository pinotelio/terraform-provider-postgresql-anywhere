...
func TestAccDatabaseFW_Basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             testAccCheckDatabaseFWDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccDatabaseFWConfig,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDatabaseFWExists("postgresql_database.mydb"),
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"alter_object_ownership"},
			},
		},
	})
}

func TestAccDatabaseFW_DefaultOwner(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             testAccCheckDatabaseFWDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccDatabaseFWConfig,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDatabaseFWExists("postgresql_database.mydb_default_owner"),
					resource.TestCheckResourceAttr(
...
func TestAccDatabaseFW_Update(t *testing.T) {
...
`, allowConnections),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDatabaseFWExists("postgresql_database.test_db"),
...
func TestAccDatabaseFW_GrantOwner(t *testing.T) {
	skipIfNotAcc(t)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					fwCheckUserMembership(t, config.Username, "test_owner", false),
				),
			},
		},
	})
...
func TestAccDatabaseFW_GrantOwnerNotNeeded(t *testing.T) {
	skipIfNotAcc(t)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					fwCheckUserMembership(t, config.Username, "test_owner", true),
				),
			},
		},
	})
...
func TestAccDatabaseFW_AlterObjectOwnership(t *testing.T) {
	skipIfNotAcc(t)
...
`,
				Check: resource.ComposeTestCheckFunc(
					...
					fwCheckTableOwnership(t, config.connStr(databaseName), new_owner, tableName),
				),
			},
		},
	})
...
func testAccCheckDatabaseFWDestroy(s *terraform.State) error {
	client := testAccProvider.Meta().(*Client)
	...
	for _, rs := range s.RootModule().Resources {
		...
		exists, err := fwCheckDatabaseExists(client, rs.Primary.ID)
...
func testAccCheckDatabaseFWExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
...
func fwCheckDatabaseExists(client *Client, dbName string) (bool, error) {
	db, err := client.Connect()
	if err != nil {
		return false, err
	}
