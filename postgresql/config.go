...
// defaultExpectedPostgreSQLVersion is the version assumed when the user has not
// set expected_version; it signals that capabilities should be fingerprinted.
const defaultExpectedPostgreSQLVersion = "9.0.0"
...
type Config struct {
	...
	// Tunnel, when set, makes the provider reach a private Host:Port through a
	// bastion and connect via a local loopback listener instead. At most one
	// transport (AWS SSM, SSH, Azure Bastion, GCP IAP) is configured.
	Tunnel Tunnel
}
...
// ForDatabase returns a copy of the client bound to the given database, letting
// callers open a connection to a database other than the provider default.
func (c *Client) ForDatabase(database string) *Client {
	return c.config.NewClient(database)
}

// DatabaseName returns the database this client connects to.
...
func (c *Client) Connect() (*DBConnection, error) {
	...
	// If a tunnel is configured, ensure it is up and connect through the local
	// loopback listener instead of the real (private) endpoint.
	if c.config.Tunnel != nil && c.config.Scheme == "postgres" {
		localHost, localPort, err := c.config.Tunnel.EnsureUp()
		if err != nil {
			return nil, fmt.Errorf("could not establish tunnel: %w", err)
		}
		c.config.Host = localHost
		c.config.Port = localPort
	}
