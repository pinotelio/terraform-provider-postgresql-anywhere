...
func TestFunctionModelToPG(t *testing.T) {
	d := mockFunctionResourceData(PGFunction{
		Name: "increment",
	...
	pgFunction := functionModelToPG(d)
...
func TestFunctionModelToPGWithArguments(t *testing.T) {
	d := mockFunctionResourceData(PGFunction{
		Name: "increment",
		Body: "BEGIN result = i + 1; END;",
		Args: []PGFunctionArg{
...
// mockFunctionResourceData builds a functionResourceModel from a PGFunction for
// exercising functionModelToPG directly.
func mockFunctionResourceData(obj PGFunction) functionResourceModel {
	model := functionResourceModel{
		Name:            types.StringValue(obj.Name),
		Returns:         types.StringValue(obj.Returns),
		Language:        types.StringValue(obj.Language),
		Body:            types.StringValue(obj.Body),
		Strict:          types.BoolValue(obj.Strict),
		SecurityDefiner: types.BoolValue(obj.SecurityDefiner),
		Parallel:        types.StringValue(obj.Parallel),
		Volatility:      types.StringValue(obj.Volatility),
	}

	for _, a := range obj.Args {
		model.Arg = append(model.Arg, functionArgModel{
			Type:    types.StringValue(a.Type),
			Name:    types.StringValue(a.Name),
			Mode:    types.StringValue(a.Mode),
			Default: types.StringValue(a.Default),
		})
	}
